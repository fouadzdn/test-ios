...
public class QRCodeScannerController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    var ChangedTickets = NSMutableArray()
//    var AllTicketArray = NSMutableArray()
...
//        self.ChangedTickets = DatabaseCalls.GetDataFromLocal(WordId: "ScannedQR") as? NSMutableArray ?? NSMutableArray()
       
        self.ChangedTickets =  delegate?.GetChangedTickets(self) as? NSMutableArray ?? NSMutableArray()
        
        print(ChangedTickets)
        if(self.ChangedTickets.count != 0)
        {
        self.navigationItem.rightBarButtonItem=UIBarButtonItem(title: "Sync \(self.ChangedTickets.count) ticket", style: .plain, target: self, action: #selector(SaveTaskButtonPressed))
        }
        

    }
    @IBAction func SaveTaskButtonPressed(_ sender: Any) {
        delegate?.SyncTickets(self)
    }
